...
static void crypt_aes_cbc_encrypt(const uint8_t* key, const uint8_t* data, size_t len, uint8_t* out)
{
    ...
    uint8_t ivec[DEF_AES_BLOCK_SIZE] = { 0 };

	EVP_EncryptInit_ex(ctx, cipher, NULL, key, ivec);

	int32_t iOutWritten = 0;
	EVP_EncryptUpdate(ctx, out, &iOutWritten, data, (int32_t)len);

	int32_t iTmplen = 0;
	EVP_EncryptFinal_ex(ctx, out + iOutWritten, &iTmplen);
    EVP_CIPHER_CTX_free(ctx);
}

static void crypt_aes_cbc_decrypt(const uint8_t* key, const uint8_t* data, size_t len, uint8_t* out, size_t* outLen)
{
    EVP_CIPHER_CTX* ctx = EVP_CIPHER_CTX_new();
    const EVP_CIPHER* cipher = EVP_aes_128_cbc();
    ...
    int32_t iOutWritten = 0;
	EVP_DecryptUpdate(ctx, out, &iOutWritten, data, (int32_t)len);
	int32_t iTotalWritten = 0;
	iTotalWritten = iOutWritten;
	EVP_DecryptFinal_ex(ctx, out + iOutWritten, &iOutWritten);
	iTotalWritten += iOutWritten;
	*outLen = iTotalWritten;
	EVP_CIPHER_CTX_free(ctx);
}
...
static int32_t lsha256(lua_State *L)
{
	size_t sz = 0;
	const void* pBuffer = luaL_checklstring(L, 1, &sz);
	uint8_t digest[DEF_SHA256_DIGEST_LENGTH];
	crypt_sha256(pBuffer, sz, digest);
	lua_pushlstring(L, (const char*)digest, DEF_SHA256_DIGEST_LENGTH);
	return 1;
}
...
static int32_t lbase64_encode(lua_State *L)
{
    ...
    uint8_t tmpBuffer[TEMP_BUFFER_SIZE] = { 0 };
    uint8_t* pEncodeBuffer = tmpBuffer;
    if (iEncodeLength > TEMP_BUFFER_SIZE)
    {
...
static int32_t lbase64_decode(lua_State *L)
{
    ...
    if (iDecodeLength > TEMP_BUFFER_SIZE)
    {
...
{
    size_t keyLen = 0;
    const uint8_t* szKey = (const uint8_t*)luaL_checklstring(L, 1, &keyLen);
	if (keyLen != 16)
	{
		return luaL_error(L, "aes_encrypt: key length must be 16 bytes");
	}
    ...
    if (sz == 0)
    {
        return luaL_error(L, "aes_encrypt: data length must be greater than 0");
    }

    size_t nEncryptLength = sz + (DEF_AES_BLOCK_SIZE - (sz % DEF_AES_BLOCK_SIZE));

    uint8_t tmpBuffer[TEMP_BUFFER_SIZE] = { 0 };
    uint8_t* pEncryptBuffer = tmpBuffer;
    if (nEncryptLength > TEMP_BUFFER_SIZE)
    {
...
static int32_t laes_decrypt(lua_State *L)
{
	...
	if (keyLen != 16)
	{
		return luaL_error(L, "aes_decrypt: key length must be 16 bytes");
	}
	
    size_t sz = 0;
    const uint8_t* pBuffer = (const uint8_t*)luaL_checklstring(L, 2, &sz);
    if (sz == 0 || sz % DEF_AES_BLOCK_SIZE != 0)
    {
        return luaL_error(L, "aes_decrypt: data length must be a multiple of 16");
    }

    uint8_t tmpBuffer[TEMP_BUFFER_SIZE] = { 0 };
    uint8_t* pDecryptBuffer = tmpBuffer;
    if (sz > TEMP_BUFFER_SIZE)
    {
        pDecryptBuffer = (uint8_t*)lua_newuserdatauv(L, sz, 0);
    }

    size_t outLen = sz;
	crypt_aes_cbc_decrypt(szKey, pBuffer, sz, pDecryptBuffer, &outLen);
    lua_pushlstring(L, (const char*)pDecryptBuffer, outLen);
    return 1;
}
