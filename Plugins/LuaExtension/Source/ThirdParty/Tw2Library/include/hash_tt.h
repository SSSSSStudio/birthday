...
// into a single hash.
static _decl_forceinline uint64_t hash_128_to_64(const uint64_t upper, const uint64_t lower) 
{
...
static _decl_forceinline uint64_t twang_mix64(uint64_t key)
{
...
static _decl_forceinline uint64_t twang_unmix64(uint64_t key) 
{
...
static _decl_forceinline uint32_t twang_32from64(uint64_t key)
{
	key = (~key) + (key << 18);
	key = key ^ (key >> 31);
	key = key * 21;
...
static _decl_forceinline uint32_t jenkins_rev_mix32(uint32_t key)
{
...
static _decl_forceinline uint32_t jenkins_rev_unmix32(uint32_t key) 
{
