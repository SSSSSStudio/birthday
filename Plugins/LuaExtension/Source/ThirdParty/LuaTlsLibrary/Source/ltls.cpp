...
static void _init_client_context(lua_State* L, struct tls_context* tls_p, struct ssl_ctx* ctx_p) 
{
...
static void _init_server_context(lua_State* L, struct tls_context* tls_p, struct ssl_ctx* ctx_p) 
{
...
static int32_t _ltls_context_finished(lua_State* L)
{
    struct tls_context* tls_p = _check_context(L, 1);
    int32_t b = SSL_is_init_finished(tls_p->ssl);
    lua_pushboolean(L, b);
    return 1;
}

static int32_t _ltls_context_close(lua_State* L) 
{
...
static int32_t _bio_read(lua_State* L, struct tls_context* tls_p) 
{
    char outbuff[4096];
    int32_t all_read = 0;
    int32_t read = 0;
    int32_t pending = BIO_ctrl_pending(tls_p->out_bio);
    if(pending >0) {
        luaL_Buffer b;
        luaL_buffinit(L, &b);
        while(pending > 0) {
...
static void _bio_write(lua_State* L, struct tls_context* tls_p, const char* s, size_t len) 
{
    char* p = (char*)s;
    size_t sz = len;
    while(sz > 0) {
        int32_t written = BIO_write(tls_p->in_bio, p, sz);
...
static int32_t _ltls_context_handshake(lua_State* L) 
{
    struct tls_context* tls_p = _check_context(L, 1);
    size_t slen = 0;
    ...
    if(!SSL_is_init_finished(tls_p->ssl)) {
        int32_t ret = SSL_do_handshake(tls_p->ssl);
        if(ret == 1) {
            return 0;
        } else if (ret < 0) {
            int32_t err = SSL_get_error(tls_p->ssl, ret);
            ERR_clear_error();
            if(err == SSL_ERROR_WANT_READ || err == SSL_ERROR_WANT_WRITE) {
                int32_t all_read = _bio_read(L, tls_p);
                if(all_read>0) {
                    return 1;
                }
            } else {
...
static int32_t _ltls_context_read(lua_State* L)
 {
    struct tls_context* tls_p = _check_context(L, 1);
    size_t slen = 0;
    ...
    do {
        ...
        if(read < 0) {
            int32_t err = SSL_get_error(tls_p->ssl, read);
            ERR_clear_error();
            if(err == SSL_ERROR_WANT_READ || err == SSL_ERROR_WANT_WRITE) {
                break;
            }
...
static int32_t  _ltls_context_write(lua_State* L) 
{
    struct tls_context* tls_p = _check_context(L, 1);
    size_t slen = 0;
    ...
    while(slen > 0) {
        int32_t written = SSL_write(tls_p->ssl, unencrypted_data,  slen);
        if(written <= 0) {
            int32_t err = SSL_get_error(tls_p->ssl, written);
            ERR_clear_error();
...
static int32_t _lset_ext_host_name(lua_State* L) 
{
    struct tls_context* tls_p = _check_context(L, 1);
    size_t slen = 0;
    char* host = (char*)lua_tolstring(L, 2, &slen);
    int32_t ret = SSL_set_tlsext_host_name(tls_p->ssl, host);
    lua_pushinteger(L, ret);
    return 1;
}

static int32_t _lctx_gc(lua_State* L) 
{
...
static int32_t _lctx_cert(lua_State* L) 
{
    struct ssl_ctx* ctx_p = _check_sslctx(L, 1);
    ...
    int32_t ret = SSL_CTX_use_certificate_chain_file(ctx_p->ctx, certfile);
    if(ret != 1) {
...
static int32_t _lctx_ciphers(lua_State* L)
{
    struct ssl_ctx* ctx_p = _check_sslctx(L, 1);
    ...
    int32_t ret = SSL_CTX_set_tlsext_use_srtp(ctx_p->ctx, s);
    if(ret != 0) {
...
int32_t luaopen_ltls(struct lua_State *L)
{
	luaL_Reg lualib_tls[] =
	{
		{"init", 	 tls_init},
