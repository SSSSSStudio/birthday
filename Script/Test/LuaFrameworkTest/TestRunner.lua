...
local TestFramework = require("Test.LuaFrameworkTest.TestFramework")
...
function TestRunner.loadTests(testModules)
    ...
    for _, modulePath in ipairs(testModules) do
        ...
        if success then
            print("[PASS] Loaded: " .. modulePath)
            loadedCount = loadedCount + 1
        else
            print("[FAIL] Failed to load: " .. modulePath)
...
function TestRunner.runTestSuite(testModules)
    ...
    if failedCount > 0 then
        print("\n[WARN] Some test modules failed to load. Aborting test run.")
        return false
    end
    
    if loadedCount == 0 then
        print("\n[WARN] No test modules loaded. Nothing to run.")
        return false
    end
