...
function TestFramework.runTest(name)
    ...
    if not testFunc then
        print("[FAIL] Test case '" .. name .. "' not found")
        return false
    end
    ...
    if isAsync then
        print("[WARN] Test '" .. name .. "' is asynchronous and cannot be run with runTest(). Use runTestAsync() or runAllTestsAsync().")
        return false
    end
    ...
    if success then
        print("[PASS] Test '" .. name .. "' passed")
    else
        print("[FAIL] Test '" .. name .. "' failed: " .. tostring(errorMsg))
    end
...
function TestFramework.runTestAsync(name, onComplete)
    ...
    print("[INFO] Running async test '" .. name .. "'...")
    ...
    if EventLoop then
        ...
        print("[WARN] Core.EventLoop not found, timeout protection disabled for async tests.")
    end
...
function TestFramework.runAllTests()
    ...
    for _, name in ipairs(testNames) do
        ...
        if not isAsync then
            ...
            print("[INFO] Skipping async test '" .. name .. "' in synchronous run.")
        end
    end
    ...
    print(string.rep("=", 60))
    print("[PASS] Passed: " .. passedCount)
            print("[FAIL] Failed: " .. failedCount)
            print("[INFO] Total:  " .. (passedCount + failedCount))
            print(string.rep("=", 60))
